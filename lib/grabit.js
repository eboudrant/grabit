...
var GUrl = function (urlStr) {
    ...
    this.transfertTo = function (writable) {
        var that = this;
        var request = http.request(this.getOptions());
        request.end();
        request.on('response', function(response) {
            console.log('STATUS: ' + response.statusCode);
            console.log('HEADERS: ' + JSON.stringify(response.headers));
            if (response.headers.location) {
                console.log('going to redirect to ' + response.headers.location + '...');
                var redirect = new GUrl(response.headers.location);
                ...
                console.log('start to transfert... ' + JSON.stringify(response.headers));
